...
#读取目标图片
target = cv2.imread("data/25m30dnoscale.png")
target_gray = cv2.cvtColor(target, cv2.COLOR_BGR2GRAY)
# target_gray = cv2.adaptiveThreshold(target_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 1, 2)

x_, y_ = int(target_gray.shape[0] / 4), int(target_gray.shape[1] / 4)
x_ = min(x_, y_)
y_ = min(x_, y_)
